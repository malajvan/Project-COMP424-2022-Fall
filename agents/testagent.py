...
class MCnodes: # check https://github.com/hayoung-kim/mcts-tic-tac-toe/blob/master/VanilaMCTS.py
        ...
        def get_children(self):
            ...
            for c in children:
                ...
                m,d=c
                child=MCnodes(m,d,self.adv_pos,root=self,chess_board=set_barrier(board,m,d))
...
def main():
    ...
    n1=MCnodes(my_pos=(1,3),barrier_dir=None,adv_pos=(3,2),value=0,chess_board=c)
    n1.get_children()
    
    n1.assign_val()
    print(n1.children.sort(key=lambda x: x.value))
    l=[]
    n1.children.sort(key=lambda x: x.value)
    for s in n1.children:
        l.append((s.my_pos,s.barrier_dir,s.value))
    print(l)
main()